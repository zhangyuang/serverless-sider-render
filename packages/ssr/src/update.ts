...
const url = 'https://registry.npm.taobao.org/ssr'
/**
 * 判断NPM包自动更新
 *
 * @export
...
export async function updateCli (): Promise<void> {
  ...
  try {
    ...
    if (latest !== localVersion) {
      const spinner = ora('发现本地版本较旧,尝试更新ssr脚手架')
      spinner.start()
      const { stdout } = await execWithPromise(`npm i -g --registry=https://registry.npm.taobao.org ssr@${latest}`)
    ...
    console.log('\x1b[91m' + `获取最新版本失败本次创建将使用本地版本${localVersion}，若版本较旧请手动安装最新版本npm i -g ssr@latest或连接代理重试`)
  }
}
